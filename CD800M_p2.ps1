...
function renamePrinter {
    ...
    #Check if printer name is already CD800M
    ...
    #If name is NOT CD800M, renames to CD800M
...
function addPrinter {
    ...
    #Adds CD800M Printer
    Add-Printer -Name "CD800M" -DriverName "XPS Card Printer" -PortName "USB DXP01 Port"
...
while ($repeatCounter -le 1) {
    ...
    foreach ($port in $listOfPorts){
        if ($port.Found -eq $true -and $port.Deleted -eq $false) {
            $listOfPorts | Format-Table | Out-file C:\Temp\CD800M_Port_Error.txt
            ...
            #controlUpSend "Unable to remove card printer port $name, details can be found in C:\Temp\CD800M_Port_Error.txt."
            $portError = $true
        }
    }
    ...
    foreach ($driver in $listOfNet){
        if ($driver.Found -eq $true -and $driver.Deleted -eq $false) {
            $listOfNet | Format-Table | Out-file C:\Temp\CD800M_NetAdapter_Error.txt
